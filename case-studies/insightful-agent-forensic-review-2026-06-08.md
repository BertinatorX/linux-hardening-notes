...
Date: 2026-06-08
System: Arch Linux 7.0.11 on AiStone X4SP4NAL (TongFang GX4), my personal daily-driver laptop
Subject: "Workpuls" / Insightful (https://insightful.io), a workforce-analytics agent
...
I installed an AppImage labeled `Workpuls.AppImage` to evaluate it as a work agent. Insightful is a workforce-analytics platform sold to employers for time tracking, application usage, and optionally screenshot/screen-record capture. I ran the installer by hand, the agent launched briefly via the menu entry, and I terminated it within minutes after deciding I should assess it before granting it standing access to my machine.

The .desktop launchers failed later on because the AppImage path I'd configured was wrong, which left the menu entries pointing at a binary that didn't exist. That forced a triage decision: clean up the failed install entirely, then review what the brief runtime had captured before re-installing it correctly with appropriate isolation.

I did the forensic walkthrough first, before re-introducing the agent in a sandboxed configuration.
...
Then the auto-restart vector check, all four returned empty:
```bash
...
Finding: no persistence vectors. The agent had no systemd user unit and no XDG autostart entry, so it would not have launched again on its own after this session.
...
Returned a 366-byte directory at `~/.config/workpuls-agent/` last modified `Jun 7 22:56`. The structure matched a textbook Electron / Chromium-derived application profile (Cookies, GPUCache, DawnCache, Local Storage, etc.). Before reading any of the contents, that told me the agent uses Electron (Chromium + Node.js packaged as a desktop app), so it would have its own embedded browser engine, separate from any system browser. It will also exhibit Chromium's full network stack, including HTTP/3, QUIC, and HSTS pinning, and standard Chromium forensic techniques would apply.
...
The mode bits on every file/dir were `0700` or `0600`. So the agent isolated its data from other users, which is fine practice, but that doesn't constrain what the agent itself can see or do as that user.

---

## Reading the Server Topology (No Code Execution Required)

The most informative artifact for a forensic walkthrough is `Network Persistent State`. This is a JSON file Chromium writes to remember HTTP/3 protocol negotiation per remote host, and reading it reveals every server the application actually attempted to contact, without ever running the app again.
...
|---|---|---|
| `app.insightful.io` | Insightful's main application server. Session and event data gets uploaded here; captured screenshots, timeline events, and idle detection all flow to this server. | Vendor, first-party data destination |
...
Conclusion: the agent communicates with three Insightful-controlled domains, plus one third-party telemetry endpoint, plus the standard Electron/Google update infrastructure. Before I grant standing access on a personal machine, these endpoints define the trust boundary I'm extending.

Branding observation: the binary is named "Workpuls" but the domains are "insightful.io". Insightful is the rebranded name of Workpuls (the rebrand was circa late 2022). The rebrand is documented publicly and nothing anomalous, but in a real triage I'd document the dual identity so it doesn't surprise me later in firewall logs.
...
This is the source IP my laptop had on the LAN at the time of capture. QUIC was used for outbound, meaning HTTP/3 over UDP 443, not the usual TCP-based HTTPS. This matters because some firewall rules built around TCP/443 will silently miss QUIC traffic.
...
Interpretation: `file counter 43` means the database has had 43 write transactions committed, which suggests modest activity during the short runtime, not silent dormancy. `database pages 116` at the default 4 KB page size is ~475 KB, and that matches the file size on disk. The real-world follow-up is to open it with `sqlite3 storage .tables` to enumerate tables. In a vendor evaluation context I would do this to understand what schemas the agent uses locally (events, screenshots, idle counters, etc.) before deciding whether the data ever leaves the machine pre-upload.
...
Only 57 bytes. No employer/team binding visible, which confirms the agent never completed a sign-in/registration handshake before I terminated it. Forensically that's the important part, because it means no employer is yet associated with my install on Insightful's backend, and a re-installation under the correct employer context would create a fresh association.

---

## Removal vs Re-installation Decision

For this evaluation I went with full removal, then a re-install with appropriate isolation when work onboarding requires it.
...
|---|---|---|---|
| Reinstall AppImage as-is | None, full access to home dir, network, processes | Lowest | No |
...
1. AppImage means no package manager and no audit trail. The OS doesn't know what an AppImage installed, what it depends on, or where its data lives. Treat every AppImage as a black box and document everything yourself.
...
5. Rebrand drift is a real OSINT problem. Workpuls becoming Insightful means searching for "Workpuls" in vulnerability databases or incident reports may miss recent issues. Always pivot from the binary's brand to the owning company to the current product name.

6. No employer association captured means a clean re-onboarding is possible. Confirming, via Preferences and Cookies inspection, that no employer binding exists means a future re-install can be done correctly with no stale identity baggage.

7. BYOD means choosing your isolation layer in advance. The right time to set up firejail, a VM, or whatever sandboxing you pick is before the work agent is reinstalled, not after. Otherwise the agent has a window of unconstrained access on first run.
